...
import {GET_ALL_BESTSELLERS_SUCCESS} from '../../../../store/actions/types';
import {getHomeProductConfig} from '../../../../services/api/homeProductServices';
import {getHomeProduct} from '../../../../store/actions/actions';
function BestSellerProduct({route, navigation}) {
  const {title} = route.params;
  const dispatch = useDispatch();
  const product = useSelector(state => state.homeProduct.allbestsellers);
  ...
  return (
    <SafeAreaView style={styles.safeViewContainer}>
      <ProductList
        data={product}
        isLoading={isLoading}
        loadMoreData={fetchDataCase}
        horizontal={false}></ProductList>
    </SafeAreaView>
  );
}
